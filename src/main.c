...
Window* window;
TextLayer *title_layer, *location_layer, *temperature_layer, *time_layer, *testkey_layer;

char location_buffer[64], temperature_buffer[32], time_buffer[32], testkey_buffer[64];
...
void process_tuple(Tuple *t)
{
	...
	switch(key) {
		...
		case KEY_TEMPERATURE:
    		...
    		case TEST_KEY:
			//Temperature received
			snprintf(testkey_buffer, sizeof("testkey: anythingnametest"), "testkey: %s", string_value);
			text_layer_set_text(testkey_layer, (char*) &testkey_buffer);
			break;
...
void window_load(Window *window)
{
	...
	testkey_layer = init_text_layer(GRect(5, 100, 190, 50), GColorBlack, GColorClear, "RESOURCE_ID_GOTHIC_18", GTextAlignmentLeft);
	text_layer_set_text(testkey_layer, "Test Key: N/A");
	layer_add_child(window_get_root_layer(window), text_layer_get_layer(testkey_layer));
};
...
{	
  ...
  text_layer_destroy(testkey_layer);
}
